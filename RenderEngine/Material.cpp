...
namespace ks {
	...
	const ShaderKey* Material::gConstantsRegistry = nullptr;
	...
	void Material::SetShaderParams()
	{
		if (ShaderContainer)
		{
			ShaderContainer->setVectorParameter(gConstantsRegistry[sci_diffuse], Diffuse);
			ShaderContainer->setVectorParameter(gConstantsRegistry[sci_ambient], Ambient);
			ShaderContainer->setVectorParameter(gConstantsRegistry[sci_emissive], Emissive);
			ShaderContainer->setVectorParameter(gConstantsRegistry[sci_specular], Specular);
			ShaderContainer->setFloatParameter(gConstantsRegistry[sci_shininess], Shininess);
		}
	}
	...
	void Material::setEmissiveLightColorOnly(Material* mat, const vec3& pEmissiveCol)
	{
